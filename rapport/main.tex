...
Hadoop's erasure coding follows a three-layer architecture that separates concerns and enables extensibility. Figure \ref{fig:layers} illustrates this layered structure with the RaptorQ integration flow:

\begin{figure}[h]
\centering
\begin{tikzpicture}[
    node distance=2cm,
    layer/.style={rectangle, draw, rounded corners=5pt, text width=5cm, text centered, minimum height=1.3cm},
    lib/.style={rectangle, draw, rounded corners=5pt, fill=orange!15, text width=5cm, text centered, minimum height=1.3cm},
    service/.style={rectangle, draw, rounded corners=5pt, fill=purple!15, text width=3cm, text centered, minimum height=1cm},
    arrow/.style={->, >=stealth, line width=1.5pt, color=black!80},
    servicearrow/.style={->, >=stealth, dashed, line width=1pt, color=purple!60}
]

    % Codec Layer
    \node[layer, fill=blue!15] (codec) {
        \textbf{Codec Layer} \\
        \texttt{RaptorQErasureCodec}
    };
    
    % Coder Layer
    \node[layer, fill=green!15, below of=codec] (coder) {
        \textbf{Coder Layer} \\
        \texttt{RaptorQEncoder/Decoder}
    };
    
    % RawCoder Layer
    \node[layer, fill=yellow!15, below of=coder] (rawcoder) {
        \textbf{RawCoder Layer} \\
        \texttt{RaptorQRawEncoder/Decoder}
    };
    
    % OpenRQ Library
    \node[lib, below of=rawcoder] (openrq) {
        \textbf{OpenRQ Library} \\
        \texttt{RFC 6330 RaptorQ}
    };
    
    % ServiceLoader (on the right)
    \node[service, right of=coder, xshift=4cm] (service) {
        \textbf{ServiceLoader} \\
        \texttt{META-INF/services/}
    };
    
    % Main vertical flow arrows
    \draw[arrow] (codec) -- (coder);
    \draw[arrow] (coder) -- (rawcoder);
    \draw[arrow] (rawcoder) -- (openrq);
    
    % ServiceLoader connections (simplified)
    \draw[servicearrow] (service.west) -- (rawcoder.east);
    \draw[servicearrow] (service.west) |- (codec.east);
    
\end{tikzpicture}
\caption{\label{fig:layers}Hadoop three-layer erasure coding architecture with RaptorQ integration. Arrows show the data flow from codec to OpenRQ library. The ServiceLoader (right) discovers factories via META-INF/services.}
\end{figure}
...
The architecture uses Java's \texttt{ServiceLoader} mechanism (shown on the right in Figure \ref{fig:layers}) for automatic factory discovery. Raw coder factories are registered in \texttt{META-INF/services/org.apache.hadoop.io.erasurecode.rawcoder.RawErasureCoderFactory} (the file contains \texttt{org.apache.hadoop.io.erasurecode.rawcoder.RaptorQRawErasureCoderFactory}), enabling automatic discovery and fallback mechanisms. Configuration maps codec names to classes via \texttt{io.erasurecode.codec.raptorq}, which defaults to \texttt{org.apache.hadoop.io.erasurecode.codec.RaptorQErasureCodec}.
...
\item Create \texttt{FECParameters} via \texttt{FECParameters.newParameters()} with data length $k \times T$, symbol size $T$, and one source block (parameter $Z=1$)
\item Initialize OpenRQ \texttt{DataEncoder} via \texttt{OpenRQ.newEncoder()} and \texttt{SourceBlockEncoder} for source block 0
\item Generate $m$ repair symbols by calling \texttt{repairPacket(ESI)} for ESIs $k$ through $k+m-1$ (deterministic selection)
\item Write each repair symbol's bytes to the corresponding parity output buffer
\end{enumerate}
...
\begin{enumerate}
\item Build \texttt{FECParameters} matching the encoding parameters (data length $k \times T$, symbol size $T$, one source block)
\item Create OpenRQ \texttt{DataDecoder} via \texttt{newDecoderWithZeroOverhead()} and \texttt{SourceBlockDecoder}
\item Feed all available symbols (data and parity) to the decoder: parse each symbol using \texttt{parsePacket()} and add it via \texttt{putEncodingPacket()}
\item Once decoding succeeds (verified by \texttt{isSourceBlockDecoded()}), extract recovered data symbols from \texttt{ArrayDataDecoder.dataArray()} for erased data blocks
\item For erased parity blocks, regenerate them by re-encoding the recovered data using a new encoder (since the decoder only provides source symbols)
\end{enumerate}

\subsection{Implementation Details}

We support both \texttt{byte[]} and \texttt{ByteBuffer} APIs for performance optimization. The ByteBuffer path enables zero-copy operations in some scenarios, while the byte array path provides simplicity and compatibility. Both paths are implemented in \texttt{RaptorQRawEncoder} and \texttt{RaptorQRawDecoder} by overriding \texttt{doEncode/doDecode} methods for both encoding state types.

ServiceLoader registration makes the factory discoverable as \texttt{raptorq\_java} (returned by \texttt{RaptorQRawErasureCoderFactory.getCoderName()}). We added a built-in erasure coding policy \texttt{RAPTORQ\_6\_3} (ID: 6) in \texttt{SystemErasureCodingPolicies.java} with schema $(k=6, m=3)$ and default cell size 1MB, making RaptorQ available through HDFS administrative tools alongside existing RS and XOR policies. Note that while \texttt{ErasureCodeConstants} defines two schemas (\texttt{RAPTORQ\_6\_3\_SCHEMA} and \texttt{RAPTORQ\_10\_4\_SCHEMA}), only the $(6,3)$ schema is registered as a built-in HDFS policy.
...
\section{Compiling}
    Hadoop provides a Dockerfile to build a Docker image with all the building tools included. We used it to get a working building environment, which include maven and ant.

    Maven is the Java building system developed by the Apache foundation, used here in Hadoop. It relies on \verb|pom.xml| files to find the dependencies and the build target in the whole project. As we added OpenRQ as a dependency we had to edit the file located in hadoop-common, the sub-project containing the erasure code.

    We also had to compile the OpenRQ library to make it available to Maven. This project is compiled with Ant, and produces a .jar archive.

    After adding OpenRQ to maven's repository, we successfuly compiled Hadoop with our own erasure code. You can find this build on our \href{https://github.com/alexandrabaron/Erasure-Coding-in-Hadoop-3.4.2/releases/tag/release}{GitHub}.

    \small{\textit{We initially relied on version 3.4.2 of Hadoop to add our code, but a bug appeared in this version because of an external package upgrade. Relevant patches from the original repository have been applied by hand, so the provided version is somewhere between 3.4.2 and 3.4.5.}}
...
Potential improvements: streaming for large configurations, padding for variable-sized chunks and production stress testing.
